...
class Player(object):
	"""
		...
		-type: the type of the player instance, for bayesian games. the framework uses Harsanyi's formulation of player types in strategic form.
	...
	"""
	def __init__(self, position, strategies, utility_function, analytic_best_response=None, type=None, beliefs=None):
	...
	def evaluate_payoff(self, player_strategy, other_players_strategy_profile):
		if self.beliefs:
			return self.utility_function(np.array(player_strategy), np.array(other_players_strategy_profile), self.type, self.beliefs)
		else:
	...
	def best_response_set(self, profile):
		if self.analytic_best_response:
			...
			self.current_best_response = best_response_set
